document.addEventListener("DOMContentLoaded", () => {
  ...
  async function fetchGitHubRepos() {
    ...
    try {
      ...
      const response = await fetch("https://api.github.com/users/Jcnok/repos?sort=updated&per_page=10")
      ...
      if (!repos || repos.length === 0) {
        projectsGrid.innerHTML = '<p class="no-projects">Nenhum repositório encontrado.</p>'
      ...
      if (filteredRepos.length === 0) {
        projectsGrid.innerHTML = '<p class="no-projects">Nenhum repositório não-fork encontrado.</p>'
        loadFallbackProjects(projectsGrid)
        return
      }
      ...
      filteredRepos.forEach((repo) => {
        ...
        const imageQuery = encodeURIComponent(
          (repo.name + " " + (repo.description || "")).replace(/[^\w\s]/gi, " ").substring(0, 30),
        )
        ...
        projectCard.innerHTML = `
          ...
          <div class="project-info">
            <h3>${displayName}</h3>
            <p>${repo.description || "Projeto de análise de dados e ciência de dados."}</p>
            <div class="project-tags">
              <span class="project-tag">${formatCategory(category)}</span>
              <span class="project-tag">${repo.language || "Python"}</span>
            </div>
            <div class="project-links">
              <a href="${repo.html_url}" target="_blank">Ver Código <i class="fas fa-code"></i></a>
              <a href="${repo.homepage || repo.html_url}" target="_blank">Demo <i class="fas fa-external-link-alt"></i></a>
            </div>
          </div>
        `
